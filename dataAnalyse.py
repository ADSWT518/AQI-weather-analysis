...
def getWeather():
    ...
    with open('changsha-air-quality.csv', newline='',
            ...
            for j in range(1, 13, 1):
                ...
                with open(str(time) + '.csv', newline='',
                    ...
                    for row2 in spamreader2:
                        ...
                        # print(wind)
                        windScale = mean(wind)
                        windList[i - 14].append(windScale)
        ...
        print(pd.value_counts([i for item in weatherList for i in item]))
...
def display2():
    ...
    for i in range(2):
        for j in range(2):
            ...
            ax.hist(pm25PerWeather, bins=40, facecolor="blue", edgecolor="black", alpha=0.7)
            print(np.mean(pm25PerWeather))
            print(np.var(pm25PerWeather))
            print(len(pm25PerWeather))
            ax.axvline(np.mean(pm25PerWeather), color='r', linestyle='dashed', linewidth=1)

            ax.set_title("PM2.5(>150) when the weather is grade " + str(i * 2 + j + 1))
